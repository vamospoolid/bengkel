...
const SingleLabel: React.FC<{ product: Product; size: string; workshopName: string; showPrice: boolean }> = ({
  ...
  return (
    ...
    >
      ...
      <p style={{ fontSize: `${cfg.fontSize + 1}px`, fontFamily: 'Arial, sans-serif', color: '#111', fontWeight: '800', margin: 0, textAlign: 'center', lineHeight: 1.1, maxWidth: '100%', overflow: 'hidden', textTransform: 'uppercase', display: '-webkit-box', WebkitLineClamp: isSmallLabel ? 1 : 3, WebkitBoxOrient: 'vertical' }}>
        {product.name}
      </p>
    ...
    if (isElectron) {
      try {
        ...
        if (size === 'label2_52x15') {
           ...
           widthMicrons = parseInt(cfg.w) * labelsPerRow * 1000;
        }
